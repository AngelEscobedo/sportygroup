...
- **Caching**: `SportsApiService` keeps one shared observable per request (`shareReplay`), so repeat calls hit the network once; failed requests are not cached, so retries work. Request budget: `1 + 2 × distinct leagues opened`.
- **UX**: an expandable card instead of a modal (the badge shows in context, one card open at a time). The Premier League is featured for a UK audience. Search is debounced (250 ms), case- and accent-insensitive. Loading, empty and error states. Accessible and responsive.
